pub mod second {
    ...
    impl Solution {
        pub fn multiply(num1: String, num2: String) -> String {
            ...
            let x: &[u8] = num1.as_bytes();
            let y: &[u8] = num2.as_bytes();

            let mut result: Vec<u8> = vec![0; x.len() + y.len()];

            for i in 0..x.len() {
                let ii: usize = x.len() - i - 1;
                let a: u8 = x[ii] - b'0';

                for j in 0..y.len() {
                    let jj: usize = y.len() - j - 1;
                    let b: u8 = y[jj] - b'0';

                    let mul = (a * b) + result[ii + jj + 1];

                    result[ii + jj + 1] = mul % 10;
                    result[ii + jj] += mul / 10;
                }
            }
