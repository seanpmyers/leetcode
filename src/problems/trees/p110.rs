pub mod recursive_2 {
    ...
    impl Solution {
        pub fn is_balanced(root: Option<Rc<RefCell<TreeNode>>>) -> bool {
            let (left, right, diff) = x(root);
            left.abs_diff(right) <= 1 && diff <= 1
        }
    }

    pub fn x(root: Option<Rc<RefCell<TreeNode>>>) -> (u16, u16, u16) {
        let Some(root) = root else {
            return (0, 0, 0);
        };
        let mut diff = 0;
        let left = x(root.borrow_mut().left.take());
        diff = diff.max(left.2);
        let left = left.0.max(left.1);

        let right = x(root.borrow_mut().right.take());
        diff = diff.max(right.2);
        let right = right.0.max(right.1);
        diff = diff.max(left.abs_diff(right));
        (left + 1, right + 1, diff)
    }
}
pub mod iterative {
